...
prediction_class <- predict(model_trained, new_data = customers_test, type = "class")
prediction_prob <- predict(model_trained, new_data = customers_test, type = "prob")
...
# accuracy 
accuracy(prediction_all, estimate = .pred_class, truth = still_customer) # 0.934

# precision
precision(prediction_all, still_customer, .pred_class) # 0.960

# recall
recall(prediction_all, still_customer, .pred_class) # 0.961

# f1 score
f_meas(prediction_all, still_customer, .pred_class) # 0.961

# log loss
mn_log_loss(prediction_all, still_customer, .pred_no) # 0.233

# calculate single-threhold sensitivity
sens(prediction_all, estimate = .pred_class, truth = still_customer) # 0.961

# calculate area under curve
roc_auc(prediction_all, estimate = .pred_no, truth = still_customer) # 0.935
